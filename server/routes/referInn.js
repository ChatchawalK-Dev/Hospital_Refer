...
router.get('/', (req, res) => {
    ...
    WHERE 
     p.hn = '${HN}' AND refer_in = 1 AND refer_out = 0 ;
    `
    , (error, results) => {
        if (error) {
            console.error(error);
...
function insertAddress(hn, res, callback) {
    pool.query(
      `INSERT INTO 
       ...
       WHERE p.hn = ? AND p.refer_in = 1 AND p.refer_out = 0;`,
       [hn],
       (error, results) => {
           if (error) {
               console.error('Error inserting address data:', error);
               callback(null, error); // Pass null as the result and error to the callback
               return;
           }
...
function insertReferInData(hn, warddisch, reason, res, callback) {
    insertAddress(hn, res, (addressId, addressError) => {
        if (addressError) {
            res.status(500).json({ error: 'Internal Server Error' });
            return;
        }

        insertContact(hn, res, (contactId, contactError) => {
            if (contactError) {
                res.status(500).json({ error: 'Internal Server Error' });
                return;
            }

            insertHealthHistory(hn, res, (healthHistoryId, healthError) => {
                if (healthError) {
                    res.status(500).json({ error: 'Internal Server Error' });
                    return;
                }

                insertDrug(hn, res, (drugId, drugError) => {
                    if (drugError) {
                        res.status(500).json({ error: 'Internal Server Error' });
                        return;
                    }
                    pool.query(
                        ...
                        FROM
                            personal_patient
                        WHERE
                            hn = ? AND refer_in = 1 AND refer_out = 0;`,  
                            [addressId, contactId, healthHistoryId, drugId, warddisch, reason, hn],
                        (error, results) => {
                            if (error) {
                            ...
                            callback(); // Call the final callback only when everything is successful
                        }
                    );
                });
            });
...
router.post('/update', (req, res) => {
    const { status, hn, warddisch, reason } = req.body;
    ...
    if (status === 1) {
        pool.query(
            'UPDATE personal_patient SET status = 1 WHERE status = 2 AND hn = ? AND refer_in = 1 AND refer_out = 0',
            [hn],
            (error, results) => {
                if (error) {
                ...
                insertReferInData(hn, warddisch, reason, res, () => {
                    res.json({ message: 'Inserted data successfully' });
                });
            }
        );
    } else if (status === 3) {
        pool.query(
            'UPDATE personal_patient SET status = 3 WHERE status = 2 AND hn = ? AND refer_in = 1 AND refer_out = 0',
            [hn],
            (error, results) => {
                if (error) {
                    console.error('Error updating status:', error);
